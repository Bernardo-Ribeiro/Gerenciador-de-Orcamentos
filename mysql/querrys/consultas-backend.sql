...
INSERT INTO clientes (nome_razao_social, cpf_cnpj, email_contato, telefone_whatsapp, status) 
VALUES (?, ?, ?, ?, 'ATIVO');

-- [RF002.02] Obtener cliente por ID
SELECT id, nome_razao_social, cpf_cnpj, email_contato, telefone_whatsapp, status, data_cadastro 
FROM clientes 
WHERE id = ?;
...
UPDATE clientes 
SET nome_razao_social = ?, cpf_cnpj = ?, email_contato = ?, telefone_whatsapp = ?, status = ? 
WHERE id = ?;
...
FROM clientes 
WHERE nome_razao_social LIKE ? AND status = 'ATIVO' 
LIMIT 10;

-- [RF003.01] Crear nuevo material/servicio
INSERT INTO materiais (nome, categoria, tipo_item, tipo_cobranca, custo_base, custo_producao, status) 
VALUES (?, ?, ?, ?, ?, ?, 'ATIVO');

-- [RF003.02] Obtener material por ID
SELECT id, nome, categoria, tipo_item, tipo_cobranca, custo_base, custo_producao, status 
FROM materiais 
WHERE id = ?;
...
UPDATE materiais 
SET nome = ?, categoria = ?, tipo_item = ?, tipo_cobranca = ?, custo_base = ?, custo_producao = ?, status = ? 
WHERE id = ?;
...
SELECT id, nome, categoria, tipo_item, tipo_cobranca, custo_base, custo_producao 
FROM materiais 
...
SELECT 
    ...
    io.area_calculada, io.valor_bruto_item, io.valor_final_item, io.custo_unitario, io.tipo_cobranca_aplicado,
    m.nome as material_nome, m.categoria, m.tipo_item, m.tipo_cobranca, m.custo_base, m.custo_producao
...
INSERT INTO itens_orcamento (id_orcamento, id_material, largura_mm, altura_mm, quantidade, area_calculada, valor_bruto_item, valor_final_item, custo_unitario, tipo_cobranca_aplicado)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);

-- [RF004.05] Actualizar item del orçamento
UPDATE itens_orcamento 
SET largura_mm = ?, altura_mm = ?, quantidade = ?, area_calculada = ?, valor_bruto_item = ?, valor_final_item = ?, custo_unitario = ?, tipo_cobranca_aplicado = ?
WHERE id = ?;
...
SELECT id, id_cliente, data_emissao, status, valor_final
FROM orcamentos 
WHERE status = ?
...
SELECT 
    ...
    u.nome as usuario, c.nome_razao_social as cliente
FROM orcamentos o
LEFT JOIN clientes c ON o.id_cliente = c.id
LEFT JOIN usuarios u ON o.id_usuario = u.id
WHERE o.id_cliente = ? 
...
SELECT 
    ...
    c.nome_razao_social as cliente, u.nome as usuario
FROM orcamentos o
LEFT JOIN clientes c ON o.id_cliente = c.id
LEFT JOIN usuarios u ON o.id_usuario = u.id
...
SELECT 
    COUNT(*) as total_orcamentos,
    SUM(CASE WHEN status = 'APROVADO' THEN 1 ELSE 0 END) as orcamentos_vendidos,
    ROUND(
        (SUM(CASE WHEN status = 'APROVADO' THEN 1 ELSE 0 END) / COUNT(*)) * 100, 
        2
...
SELECT 
    ...
    SUM(CASE WHEN o.status = 'APROVADO' THEN 1 ELSE 0 END) as orcamentos_vendidos,
    SUM(o.valor_final) as valor_total,
    ROUND(
        (SUM(CASE WHEN o.status = 'APROVADO' THEN 1 ELSE 0 END) / COUNT(o.id)) * 100,
        2
    ) as taxa_conversao
FROM orcamentos o
...
SELECT 
    ...
    COUNT(o.id) as total_orcamentos,
    SUM(CASE WHEN o.status = 'APROVADO' THEN 1 ELSE 0 END) as ventas,
    SUM(o.valor_final) as valor_total_emitido,
    SUM(CASE WHEN o.status = 'APROVADO' THEN o.valor_final ELSE 0 END) as valor_vendido
FROM orcamentos o
